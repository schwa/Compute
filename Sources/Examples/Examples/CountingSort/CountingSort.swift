...
struct CountingSortDemo {
    ...
    func countingSort1(dispatch: Compute.Dispatcher, input: TypedMTLBuffer<UInt32>) throws -> TypedMTLBuffer<UInt32> {
        let device = compute.device
        let output: TypedMTLBuffer<UInt32> = try device.makeTypedBuffer(count: input.count)
        try countingSort(dispatch: dispatch, input: input, output: output, shift: 0)
        return output
    }
        ...
        let output = try device.capture(enabled: Self.capture) {
            try compute.task { task in
                try task { dispatch in
                    try sort.countingSort1(dispatch: dispatch, input: input)
                }
            }
        }
